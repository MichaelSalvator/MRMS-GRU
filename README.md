# TSExplorer: Time-Series Remote Sensing Mangrove Classification and Change Detection Framework

This project is a pixel-level classification and change detection framework for mangroves based on time-series multispectral remote sensing imagery (e.g., Sentinel-2). The repository includes traditional machine learning baseline models (Random Forest, XGBoost, SVM) and proposes a custom deep learning architecture named **TSExplorer**, which integrates multi-scale 1D Convolutional Neural Networks (1D-CNN) and Bidirectional Gated Recurrent Units (BiGRU).
...
* **Model and Data Loading Module**
  ...
  * `model3.py`: The core deep learning model script that defines the **TSExplorer** network. It includes multi-scale convolutional feature extractors, residual connections, and sequence modeling layers.
